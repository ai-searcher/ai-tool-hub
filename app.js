...
// ==================== NEON-HILFE-BUTTON FÜR VERBINDUNGEN (MEHRSPRACHIG) ====================
function initNeonHelp() {
  const helpBtn = document.getElementById('neonHelpBtn');
  const popup = document.getElementById('neonPopup');
  if (!helpBtn || !popup) return;

  function updatePopupContent() {
    if (!window.i18n) return;
    popup.innerHTML = `
      <h4>${window.i18n.t('neonHelpTitle')}</h4>
      <ul>
        <li><span class="legend-line solid"></span> <strong>${window.i18n.t('neonHelpStrong')}</strong></li>
        <li><span class="legend-line dashed"></span> <strong>${window.i18n.t('neonHelpWeak')}</strong></li>
        <li><span class="legend-line dotted"></span> <strong>${window.i18n.t('neonHelpBridge')}</strong></li>
      </ul>
      <p>${window.i18n.t('neonHelpColorHint')}</p>
    `;
  }
  ...
  // Bei Sprachwechsel aktualisieren
  window.addEventListener('languagechange', updatePopupContent);
